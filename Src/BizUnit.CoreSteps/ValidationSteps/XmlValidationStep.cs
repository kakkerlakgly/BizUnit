...
namespace BizUnit.CoreSteps.ValidationSteps
{
    ...
    ///             <description>
    ///                 The XSD schema to use to validate the XML data
    ///                 <para>(optional)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    ///             <description>
    ///                 The XSD schema namespace to validate the XML data against
    ///                 <para>(optional)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    ///             <description>
    ///                 XPath expression to evaluate against the XML document is defined in the attribute query, the
    ///                 expected result is defined in the element
    ...
    public class XmlValidationStep : IValidationStepOM
    {
        ...
        public void ExecuteValidation(Stream data, XmlNode validatorConfig, Context context)
        {
            ...
            {
                var xpathExp = xpath.SelectSingleNode("@query").Value;
                var expectedValue = xpath.SelectSingleNode(".").InnerText;
        ...
        public void ExecuteValidation(Stream data, Context context)
        {
            ...
            foreach (var validation in XPathValidations)
            {
                ...
                var checkNode = doc.SelectSingleNode(xpathExp);
                ...
                if (0 != expectedValue.CompareTo(checkNode.InnerText))
                {
                    throw new InvalidOperationException(
                        string.Format("XmlValidationStep failed, compare {0} != {1}, xpath query used: {2}",
                            expectedValue, checkNode.InnerText, xpathExp));
                }
            }
        }
