...
namespace BizUnit.CoreSteps.ValidationSteps
{
    ...
    ///             <description>The regualar expression to be evaluated
    ///                 <para>(repeating)</para>
    ///             </description>
    ///         </item>
    ///     </list>
    ...
    public class RegExValidationStep : IValidationStepOM
    {
        ...
        {
            var validationNodes = validatorConfig.SelectNodes("ValidationRegEx");
        ...
        public void ExecuteValidation(Stream data, Context context)
        {
            ...
            foreach (var validationRegEx in ValidationRegEx)
            {
                var match = Regex.Match(strData, validationRegEx);
                ...
                {
                    throw new InvalidOperationException(string.Format("Regex validation failed for pattern \"{0}\".",
                        validationRegEx));
                }
            }
        }
