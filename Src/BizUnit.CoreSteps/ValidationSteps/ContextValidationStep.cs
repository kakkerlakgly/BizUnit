...
namespace BizUnit.CoreSteps.ValidationSteps
{
    ...
    ///             <term>Context</term>
    ///             <description>
    ///                 The attribute keyName identifies the context key to validate, while the element represents the
    ///                 expected result
    ///                 <para>(optional)(repeating)</para>
    ///             </description>
    ///         </item>
    ///     </list>
    /// </remarks>
    ...
    public class ContextValidationStep : IValidationStep
    {
        ...
        {
            var validationNodes = validatorConfig.SelectNodes("Context");

            foreach (XmlNode validationNode in validationNodes)
            {
                var keyName = validationNode.SelectSingleNode("@keyName").Value;
                var expectedValue = validationNode.InnerText;

                var actualValue = context.GetValue(keyName);

                if ((!string.IsNullOrEmpty(actualValue)) && (0 == expectedValue.CompareTo(actualValue)))
                {
                    context.LogInfo("Context validation succeeded, key: \"{0}\" equals: \"{1}\".", keyName,
                        expectedValue);
                }
                else
                {
                    throw new InvalidOperationException(
                        string.Format("Context validation failed for key: {0}. Expected value: {1}, actual value: {2}",
                            keyName, expectedValue, actualValue));
                }
            }
        }
    }
