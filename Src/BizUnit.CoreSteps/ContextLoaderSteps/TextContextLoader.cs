...
namespace BizUnit.CoreSteps.ContextLoaderSteps
{
    ...
    public class TextContextLoader : IContextLoaderStep
    {
        ...
        {
            var contextNodes = contextConfig.SelectNodes("Item");
            ...
            {
                var contextKey = contextNode.SelectSingleNode("@contextKey").Value;
                var searchString = contextNode.SelectSingleNode("@searchString").Value;
                var skipNumber = Convert.ToInt32(contextNode.SelectSingleNode("@skipNumber").Value);
                var stringLength = Convert.ToInt32(contextNode.SelectSingleNode("@stringLength").Value);

                var startIndex = strData.IndexOf(searchString);
                var targetData = strData.Substring(startIndex + skipNumber + searchString.Length, stringLength);
