...
namespace BizUnit.CoreSteps.TestSteps
{
    ...
    ///             <term>ItemTest</term>
    ///             <description>
    ///                 The item to append, note that a value from the context maybe used if the attribute takeFromCtx
    ///                 is used
    ...
    public class ContextManipulatorStep : ITestStep
    {
        ...
        {
            var ctxItems = testConfig.SelectNodes("ContextItem");

            foreach (XmlNode ctxItem in ctxItems)
            {
                var newCtxNode = ctxItem.SelectSingleNode("@contextKey").Value;

                var items = ctxItem.SelectNodes("ItemTest");

                var newValue = items.Cast<XmlNode>()
                    .Aggregate("", (current, item) => current + context.ReadConfigAsString(item, "."));
