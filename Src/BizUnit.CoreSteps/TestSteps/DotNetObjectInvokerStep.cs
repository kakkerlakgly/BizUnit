...
namespace BizUnit.CoreSteps.TestSteps
{
    ...
    ///             <TypeName>
    ///                 Microsoft.Samples.BizTalk.WoodgroveBank.ServiceLevelTracking.ServiceLevelTracking,
    ///                 Microsoft.Samples.BizTalk.WoodgroveBank.ServiceLevelTracking, Version=1.0.0.0, Culture=neutral,
    ///                 PublicKeyToken=a1054514fc67bded
    ///             </TypeName>
    ...
    ///             <term>TypeName</term>
    ///             <description>
    ///                 The name of the type of the .Net object to invoke the method on. Note, if the type is GAC'd
    ///                 then the assembly name, version, public key etc need to be specified
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    ///             <term>Parameter</term>
    ///             <description>
    ///                 The value for the parameter to pass into the method. Note: the format should be the serialised .Net
    ///                 type,
    ///                 if the value is taken form the context, the value in the context should also be a serialized .Net type.
    ...
    public class DotNetObjectInvokerStep : ITestStep
    {
        ...
        public void Execute(XmlNode testConfig, Context context)
        {
            ...
            var paramsForCall =
                pi.Select(t1 => t1.ParameterType)
                    .Select(t => new XmlSerializer(t))
                    .Select(
                        (xs, c) =>
                            xs.Deserialize(
                                new XmlTextReader(StreamHelper.LoadMemoryStream(context.GetInnerXml(parameters[c])))))
                    .ToArray();
        ...
        private static object CreateObject(string typeName, string assemblyPath, Context context)
        {
            object comp = null;
            Type ty;
