...
namespace BizUnit.CoreSteps.TestSteps
{
    ...
    ///             <CountersListFilePath>
    ///                 C:\Program Files\BizUnit
    ///                 2.1\Src\SDK\BizUnitFunctionalTests\TestData\Test_08_PerfCounters.txt
    ///             </CountersListFilePath>
    ///             <SampleInterval>5</SampleInterval>
    ///             <PerfmonLogFilePath>
    ///                 C:\Program Files\BizUnit
    ///                 2.1\Src\SDK\BizUnitFunctionalTests\TestData\BTS_Run01_%DateTime%.blg
    ///             </PerfmonLogFilePath>
    ...
    ///             <description>
    ///                 The name of the perfmon counters to start recording
    ///                 <para>(only required if PerfmonAction=Start)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <term>CountersListFilePath</term>
    ///             <description>
    ///                 The file containing the set of perfmon counters to record
    ///                 <para>(only required if PerfmonAction=Start)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    internal class PerfmonCountersStep : ITestStep
    {
        /// <summary>
        ...
        {
            var perfmonAction = context.ReadConfigAsString(testConfig, "PerfmonAction");
            var counterSetName = context.ReadConfigAsString(testConfig, "CounterSetName");
            var workingDirectory = Environment.CurrentDirectory;

            switch (perfmonAction)
            {
                case "Start":
                    var countersListFilePath = context.ReadConfigAsString(testConfig, "CountersListFilePath");
                    var perfmonLogFilePath = context.ReadConfigAsString(testConfig, "PerfmonLogFilePath");
                    var sampleInterval = context.ReadConfigAsString(testConfig, "SampleInterval");
                    var userName = context.ReadConfigAsString(testConfig, "UserName", true);
                    var passWord = context.ReadConfigAsString(testConfig, "PassWord", true);
                    ...
                    context.LogInfo(
                        "PerfmonCountersStep is about to start the perfmon counter set : \"{0}\" writting log file:\"{1}\"",
                        counterSetName, perfmonLogFilePath);
                ...
                case "Stop":
                    context.LogInfo("PerfmonCountersStep is about to stop the perfmon counter set : \"{0}\"",
                        counterSetName);
                ...
                default:
                    throw new InvalidOperationException(
                        string.Format("PerfmonCountersStep does not recognise PerfmonAction= {0}", perfmonAction));
            }
        }
        ...
        private static void ExecuteLogman(string workingDirectory, string processParams, Context context)
        {
            using (var process = new Process())
            {
                ...
                if (0 != exitCode)
                {
                    throw new InvalidOperationException(
                        string.Format(
                            "PerfmonCountersStep received an exit code: {0} while executing process {1}\n\nOutput: {2}",
                            exitCode, processParams, output));
                }
            }
        }
    }
