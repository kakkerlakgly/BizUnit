...
{
    /// <summary>
    ///     The DatabaseRowCountStep step uses the supplied connection string, table name and condition to determine the number
    ...
    public class DatabaseRowCountStep : ITestStep
    {
        ...
        public void Execute(XmlNode testConfig, Context context)
        {
            ...
            var table = context.ReadConfigAsString(testConfig, "Table");
            var condition = context.ReadConfigAsString(testConfig, "Condition");
            var expectedRows = context.ReadConfigAsInt32(testConfig, "ExpectedRows");

            // Build SQL statement
            var sqlStatement = "select count(*) from " + table + " where " + condition;

            context.LogInfo("DatabaseRowCountStep connecting to {0}, executing statement {1}", connectionString,
                sqlStatement);

            // Execute command against specified database
            var rows = DatabaseHelper.ExecuteScalar(connectionString, sqlStatement);

            // Number of rows as expected?
            if (rows != expectedRows)
            {
                throw new InvalidOperationException(
                    string.Format("DatabaseRowCountStep failed, expected {0} rows but found {1} rows", expectedRows,
                        rows));
            }
