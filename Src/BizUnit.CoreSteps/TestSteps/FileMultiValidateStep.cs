...
namespace BizUnit.CoreSteps.TestSteps
{
    ...
    ///                 <XPathList>
    ///                     <XPathValidation
    ///                         query="/*[local-name()='PurchaseOrder' and namespace-uri()='http://SendMail.SalesOrder']/*[local-name()='PONumber' and namespace-uri()='']">
    ///                         PONumber_0
    ///                     </XPathValidation>
    ///                 </XPathList>
    ///             </ValidationStep>
    ...
    ///             <description>
    ///                 The validation step to use against the files
    ...
    public class FileMultiValidateStep : ITestStep
    {
        ...
        public void Execute(XmlNode testConfig, Context context)
        {
            ...
            var directoryPath = context.ReadConfigAsString(testConfig, "DirectoryPath");
            var pattern = context.ReadConfigAsString(testConfig, "SearchPattern");
            var filelist = Directory.GetFiles(directoryPath, pattern);
            ...
            var validationConfigs = testConfig.SelectNodes("ValidationStep");
            var nullException = false;
            var foundSteps = 0;
            ...
            foreach (var filePath in filelist)
            {
                ...
                for (var i = 0; i < validationConfigs.Count; i++, nullException = false)
                {
                    try
                    {
            ...
            {
                throw new MultiUnknownException(
                    "FileMultiValidateStep failed, did not match all the files to those specifed in the validate steps.");
            }
        }
    }
}