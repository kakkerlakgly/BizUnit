...
namespace BizUnit.CoreSteps.TestSteps
{
    ...
    ///             <description>The perfmon counter instance name
    ///                 <para>(optional)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ...
    ///         <item>
    ///             <term>SleepTime</term>
    ///             <description>The length of the delay in miliseconds between checks of the counter
    ///                 <para>(optional)</para>
    ///             </description>
    ///         </item>
    ///         <item>
    ///             <term>TimeOut</term>
    ///             <description>The maximum length of time the step will spend checking the counter (seconds)
    ///                 <para>(optional)</para>
    ///             </description>
    ///         </item>
    ///     </list>
    ...
    public class PerfMonCounterMonitorStep : ITestStep
    {
        ...
        {
            // Read config...
            var categoryName = context.ReadConfigAsString(testConfig, "CategoryName");
            var counterName = context.ReadConfigAsString(testConfig, "CounterName");
            var instanceName = context.ReadConfigAsString(testConfig, "InstanceName", true);
            var server = context.ReadConfigAsString(testConfig, "Server");
            var counterTargetValue = context.ReadConfigAsFloat(testConfig, "CounterTargetValue");
            var sleepTime = context.ReadConfigAsInt32(testConfig, "SleepTime", true);
            var timeOut = context.ReadConfigAsInt32(testConfig, "TimeOut", true);

            context.LogInfo("About to start monitoring: {0}\\{1}\\{2}({3}) for the target value: {4}", server,
                categoryName, counterName, instanceName, counterTargetValue);
