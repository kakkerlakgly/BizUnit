...
namespace BizUnit
{
    ...
    /// </summary>
    /// <remarks>
    ///     The context is passed by BizUnit to each individual test step, state maybe written to or read from the context,
    ///     note the following
    ///     wild cards are supported: %DateTime%, %DateTimeISO8601%, %ServerName% and %Guid%.
    ///     The context also provides helper methods for test steps to read their configuration, and to log information,
    ///     warnings, errors
    ///     and data in a consistent manner.
    ///     The context contains some standard information on the current test executing, such as BizUnitTestCaseName which
    ///     contains the
    ///     name of the current test case being executed and BizUnitTestCaseStartTime which contains the time the current test
    ///     case started.
    /// </remarks>
    public class Context
    {
        ...
        internal BizUnit BizUnitObject { get; private set; }

        /// <summary>
        ///     Sets a value indicating whether members, which implement <see cref="IDisposable" />, are disposed on test case
        ///     completion.
        /// </summary>
        /// <value>
        ...
        {
            set { _disposeMembersOnTestCaseCompletion = value; }
        }
        ...
        {
            return new Context(BizUnitObject, _context, logger, _startTime);
        }
        ...
        /// <param name="key">The name of the key for the object added.</param>
        /// <param name="newValue">The object to be added to the context.</param>
        /// <param name="updateIfExists">
        ///     If the object already exists and this flag
        ...
        {
            BizUnitObject.ExecuteValidator(data, validationStep, this);
        }
        ...
        /// </summary>
        /// <param name="data">The stream containing the data to validate.</param>
        /// <param name="validatorConfig">
        ///     The Xml configuration for the validation test step, this configuration is passed onto the
        ///     validation step by BizUnit.
        /// </param>
        /// <remarks>
        ...
        {
            BizUnitObject.ExecuteValidator(data, validatorConfig, this);
        }
        ...
        /// <summary>
        ///     Executes a validation test step, either the Xml configuration for the step or the BizUnit OM maybe supplied, but
        ///     not both.
        /// </summary>
        /// <param name="data">The stream containing the data to validate.</param>
        /// <param name="validatorConfig">
        ///     The Xml configuration for the validation test step, this configuration is passed onto the
        ///     validation step by BizUnit (optional).
        /// </param>
        ...
        ///     The Xml configuration for the context loader
        ///     test step, this configuration is passed onto the validation step by BizUnit (optional).
        /// </param>
        /// <remarks>
        ///     Context loader steps load data onto the context. The following example demonstrates how to execute a validation
        ///     test step:
        ///     <code escaped="true">
        ...
        {
            BizUnitObject.ExecuteContextLoader(data, contextConfig, this);
        }
        ...
        /// <param name="data">The stream containing the data to validate.</param>
        /// <param name="contextConfig">
        ///     The Xml configuration for the context loader step, this configuration is passed onto the
        ///     context loader step by BizUnit (optional).
        /// </param>
        ...
        /// <param name="contextConfig">
        ///     The Xml configuration for the context loader
        ///     test step, this configuration is passed onto the validation step by BizUnit.
        /// </param>
        /// <param name="seekStream">True if the stream should be seeked to the begining.</param>
        /// <remarks>
        ///     Context loader steps load data onto the context. The following example demonstrates how to execute a validation
        ...
        /// </summary>
        /// <param name="data">The stream containing the data to validate.</param>
        /// <param name="contextLoaderStep">
        ///     The context loader step implmenting IContextLoaderStepOM which will be executed
        ///     (optional).
        /// </param>
        /// <param name="seekStream">True if the stream should be seeked to the begining.</param>
        [Obsolete("ExecuteContextLoader has been deprecated.")]
        public void ExecuteContextLoader(Stream data, IContextLoaderStepOM contextLoaderStep, bool seekStream = false)
        {
            ...
            BizUnitObject.ExecuteContextLoader(data, contextLoaderStep, this);
        }
        ...
        /// <param name="optional">
        ///     True if this is an option field. If false and the XPath expression does not find a value and
        ...
        /// </remarks>
        [Obsolete(
            "ReadConfigAsXXX has been deprecated. From 4.0 test steps are configured using property setters and getters."
            )]
        ...
        private object ReadConfigAsObject(XmlNode config, string xPath, bool optional, bool asXml)
        {
            ...
            if (null != node)
            {
                ...
                var fromCtx = node.SelectSingleNode("@takeFromCtx");
                if (null != fromCtx)
                {
                    var s = fromCtx.Value;
                    if (0 < s.Length)
            ...
            throw new InvalidOperationException(string.Format("The XPath query: {0} did not find a node.", xPath));
        }
        ...
        public object ReadArgument(object arg)
        {
            ...
            if (s != null)
            {
                ...
                {
                    var key = strArg.Substring(TAKE_FROM_CONTEXT.Length);
                    object value;
                    if (_context.TryGetValue(key, out value)) return value;
                    return null;
                }
        ...
        public string SubstituteWildCards(string rawString)
        {
            ...
            if (result.Contains(TEST_START_DATE_TIME))
            {
                var testStartTime = (DateTime) _context[BizUnit.BizUnitTestCaseStartTime];
                result = result.Replace(TEST_START_DATE_TIME, testStartTime.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            }
