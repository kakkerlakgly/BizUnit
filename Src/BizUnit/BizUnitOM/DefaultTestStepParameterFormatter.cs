...
namespace BizUnit.BizUnitOM
{
    ...
    public class DefaultTestStepParameterFormatter : ITestStepParameterFormatter
    {
        ...
        public object[] FormatParameters(Type type, object[] args, Context ctx)
        {
            ...
            {
                retVal = new object[] {Convert.ToDouble(ctx.ReadArgument(args[0]))};
            }
            else if (typeof (Boolean) == type)
            {
                retVal = new object[] {Convert.ToBoolean(ctx.ReadArgument(args[0]))};
            }
            else if (typeof (Int16) == type)
            {
                retVal = new object[] {Convert.ToInt16(ctx.ReadArgument(args[0]))};
            }
            else if (typeof (Int32) == type)
            {
                retVal = new object[] {Convert.ToInt32(ctx.ReadArgument(args[0]))};
            }
            else if (typeof (Int64) == type)
            {
                retVal = new object[] {Convert.ToInt64(ctx.ReadArgument(args[0]))};
            }
            else if (typeof (String) == type)
            {
                retVal = new object[] {ctx.ReadArgument(args[0])};
            }
