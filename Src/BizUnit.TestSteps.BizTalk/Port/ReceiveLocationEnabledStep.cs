...
{
    /// <summary>
    ///     The ReceiveLocationEnabledStep test step checks to determine whether a specific receive location is enabled or
    ///     disabled.
    /// </summary>
    /// <remarks>
    ...
    ///             typeName="BizUnit.BizTalkSteps.ReceiveLocationEnabledStep, BizUnit.BizTalkSteps, Version=3.1.0.0, Culture=neutral, PublicKeyToken=7eb7d82981ae5162">
    ...
    ///             <description>
    ///                 If true is specified then the test step will check to see that the receive location is
    ///                 disabled, if false, the step will check it is enabled
    ///             </description>
    ///         </item>
    ///     </list>
    /// </remarks>
    public class ReceiveLocationEnabledStep : TestStepBase
    {
        ...
        /// <summary>
        ///     If true is specified then the test step will check to see that the receive location is disabled, if false, the step
        ///     will check it is enabled
        /// </summary>
        ...
        public override void Execute(Context context)
        {
            ...
            if (IsDisabled != isActualDisabled)
            {
                throw new InvalidOperationException(
                    string.Format("The receive location: {0} was found to not be in the state IsDisabled = {1}",
                        ReceiveLocationName, IsDisabled));
            }
            context.LogInfo(string.Format("The receive location: {0} was found to be in the state IsDisabled = {1}",
                ReceiveLocationName, IsDisabled));
        }
        ...
        private static ManagementObject GetreceiveLocationWmiObject(string receiveLocation)
        {
            using (var searcher = new ManagementObjectSearcher())
            {
                ...
                {
                    throw new InvalidOperationException(
                        string.Format("The WMI object for the receive location:{0} could not be retrieved.",
                            receiveLocation));
                }
            }
        }
