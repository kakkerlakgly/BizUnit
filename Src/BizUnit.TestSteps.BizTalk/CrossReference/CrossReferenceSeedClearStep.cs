...
namespace BizUnit.TestSteps.BizTalk.CrossReference
{
    ...
    ///             typeName="BizUnit.BizTalkSteps.CrossReferenceSeedClearStep, BizUnit.BizTalkSteps, Version=3.1.0.0, Culture=neutral, PublicKeyToken=7eb7d82981ae5162">
    ///             <LoginId>sa</LoginId>
    ...
    public class CrossReferenceSeedClearStep : ITestStep
    {
        ...
        public void Execute(XmlNode testConfig, Context context)
        {
            ...
            string server;
            using (
                var rk = Registry.LocalMachine.OpenSubKey("Software\\Microsoft\\BizTalk Server\\3.0\\Administration\\"))
            {
                dbname = rk.GetValue("MgmtDBName").ToString();
                server = rk.GetValue("MgmtDBServer").ToString();
            }
            var connection = "Initial Catalog=" + dbname + " ;Data Source=" + server + ";Integrated Security=SSPI;";
            var loginNode = testConfig.SelectSingleNode("LoginId");
            var passwordNode = testConfig.SelectSingleNode("password");

            if (loginNode != null && passwordNode != null)
            {
                connection = "Initial Catalog=" + dbname + " ;Data Source=" + server + ";User Id=" + loginNode.InnerText +
